#!/usr/bin/env python3
"""Convergence sweep required by P2.3.2 (16/32/64/128/256 updates)."""
...
METHODS = ("GRPO-Atomic", "Atomic+Flip", "Atomic+Branch", "PESCO-Full")
...
def main(argv=None):
    p = argparse.ArgumentParser(); p.add_argument("--dataset", type=Path, default=ROOT/"artifacts/tier1_p23_promotion_v2/dataset_raw_evidence.json"); p.add_argument("--output-dir", type=Path, default=ROOT/"artifacts/tier1_p232_convergence"); p.add_argument("--seeds", type=int, nargs="+", default=(17,23,29)); p.add_argument("--steps", type=int, nargs="+", default=(16,32,64,128,256)); p.add_argument("--batch-size", type=int, default=32)
    ...
    for steps in a.steps:
        out=a.output_dir/f"steps_{steps}"; cfg=P231Config(sft_steps=16, finetune_steps=steps, batch_size=a.batch_size, hidden_dim=24, learning_rate=2e-3, minibatch_epochs=2)
    ...
    for method, rows in by_method.items():
        ...
        if len(rows)>=3:
            tail=rows[-3:]; xs=[r["steps"] for r in tail]; ys=[r["tune_mean_regret"] for r in tail]; slope=sum((x-sum(xs)/len(xs))*(y-sum(ys)/len(ys)) for x,y in zip(xs,ys))/max(1e-9,sum((x-sum(xs)/len(xs))**2 for x in xs)); spread=max(ys)-min(ys); rows[-1]["tail_regret_slope"] = slope; rows[-1]["tail_regret_spread"] = spread; rows[-1]["plateau_gate"] = abs(slope) < 1e-4 and spread < 0.02
