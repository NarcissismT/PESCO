...
def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument("--result",type=Path,required=True); p.add_argument("--output",type=Path,required=True); p.add_argument("--bootstrap-replicates",type=int,default=5000); a=p.parse_args(argv)
    ...
    for m in methods:
        for row in records:
            if row.get("method")!=m or row.get("split")!="tune": continue
